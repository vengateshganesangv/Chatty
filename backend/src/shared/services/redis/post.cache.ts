...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    try {
      ...
      for (let data = 0; data < dataToSave.length; data += 2) {
        const data_key = dataToSave[data];
        const data_value = dataToSave[data + 1];
        multi.HSET(`posts:${key}`, data_key, data_value);
      }
  ...
  public async updatePostInCache(key: string, updatedPost: IPostDocument): Promise<IPostDocument> {
    ...
    try {
      ...
      for (let data = 0; data < dataToSave.length; data += 2) {
        const data_key = dataToSave[data];
        const data_value = dataToSave[data + 1];
        await this.client.HSET(`posts:${key}`, data_key, data_value);
      }
      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
  ...
  public async getPostsWithVideosFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      const reply: string[] = await this.client.ZRANGE(key, start, end, { REV: true, BY: 'SCORE' });
      const multi: ReturnType<typeof this.client.multi> = this.client.multi();
      for (const value of reply) {
        multi.HGETALL(`posts:${value}`);
      }
